...
def main():
    ...
    # converting borrowable eth to dai * 95%
    print(f"Print we are going to borrow {amount_dai_to_borrow} DAI")
    # Now we will borrow
...
def repay_all(amount, lending_pool, account):
    ...
    approve_erc20(Web3.toWei(
        amount, 'ether'),
        lending_pool.address, dai_token,
        account
    )
    repay_tx = lending_pool.repay(dai_token,
                                  amount, 1,
                                  account.address,
                                  {'from': account}
                                  )
...
def get_borrowable_data(lending_pool, account):
    ...
    print(f"You have {total_collateral_eth} worth of deposited")
    print(f"You have {total_debt_eth} worth of borrowed")
...
def approve_erc20(amount, spender, erc20_address, account):
    ...
    tx = erc20.approve(spender, amount, {"from": account.address})
    tx.wait(1)
    print("Approved!")
    return tx
